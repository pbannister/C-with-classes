...
class string_local_o {
protected:
    char* p_buffer = 0;
    unsigned n_room = 0;

private:
    unsigned id_thread = 0;
...
public:
    ...
    void operator=(const string_local_o& s) {
        strcpy(s);
    }
...
public:
    ...
    char* buffer_get(unsigned n) {
        return ensure_room_have(n, p_buffer, ::strlen(p_buffer));
    }
...
protected:
    char* ensure_room_have(unsigned, const char*, unsigned);
    char* ensure_room_need(unsigned, const char*, unsigned);
    char* expand_room(unsigned, const char*, unsigned);

public:
    string_local_o();
    string_local_o(const char*);
    string_local_o(const string_local_o&);
    ~string_local_o();
