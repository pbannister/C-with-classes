...
void report_scores() {
    ...
    string_local_o::scorecard_o o;
    string_local_o::scorecard_get(o);
    ::printf(
        "1 new : %8u free : %8u\n"
        "2 new : %8u free : %8u\n",
        o.n1_new, o.n1_free,
...
bool bump_is(
    const string_local_o::scorecard_o& a,
    const string_local_o::scorecard_o& b,
...
//
//  The scorecard is compiled out unless asked for, so assertions that
//  inspect it are compiled out with it.
//
#if WANT_STRING_SCORECARD
#define ASSERT_BUMP(X) ASSERT_IF(X)
#define SCORECARD_GET(S) string_local_o::scorecard_get(S)
#else
#define ASSERT_BUMP(X) ((void) 0)
#define SCORECARD_GET(S) ((void) 0)
#endif

bool test_1() {
    ::printf("\n==== test 1\n");
#if WANT_STRING_SCORECARD
    string_local_o::scorecard_o sc1;
    string_local_o::scorecard_o sc2;
#endif

    SCORECARD_GET(sc1);
    SCORECARD_GET(sc2);
    ASSERT_BUMP(bump_is(sc1, sc2, 0, 0, 0, 0));

    SCORECARD_GET(sc1);
    string_local_o s1;
    SCORECARD_GET(sc2);
    ASSERT_BUMP(bump_is(sc1, sc2, 1, 0, 0, 0));
    ...
    SCORECARD_GET(sc2);
    ASSERT_BUMP(bump_is(sc1, sc2, 1, 1, 0, 0));
    ...
    {
        SCORECARD_GET(sc1);
        string_local_o a1[20];
        for (auto i = 0; i < 20; ++i) {
            a1[i] = "1999";
            a1[i].strcat(s1);
        }
        SCORECARD_GET(sc2);
        ASSERT_BUMP(bump_is(sc1, sc2, 20, 0, 0, 0));
    }
    SCORECARD_GET(sc2);
    ASSERT_BUMP(bump_is(sc1, sc2, 20, 20, 0, 0));
    {
        SCORECARD_GET(sc1);
        string_local_o a1[20];
        for (auto i = 0; i < 20; ++i) {
