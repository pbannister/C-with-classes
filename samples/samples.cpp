...
void samples_base_o::samples_print(const char* s_prefix, unsigned n) {
    unsigned t = 0;
...
samples_bucket_o::~samples_bucket_o() {
    delete[] p_bucket;
    p_bucket = 0;
    n_space = 0;
}

const char* samples_bucket_o::sample_pick() const {
...
bool samples_bucket_o::bucket_fill(unsigned n_average, unsigned q) {
...
bool samples_simple_o::sample_fill(unsigned n) {
    // Translate subindex to printable character.
    char xlat[256];
    {
        for (unsigned i = 0; i < 256; ++i) {
...
const char* samples_simple_o::sample_pick() const {
    unsigned i = (::rand() % n_sample);
    return p_sample + i;
}

samples_simple_o::~samples_simple_o() {
    delete[] p_sample;
    p_sample = 0;
    n_sample = 0;
}
